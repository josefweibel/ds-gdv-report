...
\begin{titlepage}
    \begin{center}
        ...
        \includegraphics[width=0.75\textwidth]{title-image}
...
\small
Visualisation on the title page has a decorative purpose only.
\normalsize
...
```{r map, warning=F, fig.asp=0.45, out.width=".25\\textwidth", wrapfigure=list("R", 0.25), fig.cap=map_caption}
map_caption <- 'Map showing voter participation per canton.'
plot_map <- function () {
...
```{r location_scale, fig.asp=0.18, out.width="50%", fig.show = "hold", fig.cap=location_scale_caption}
location_scale_caption <- 'Line charts showing voter participation over time with different y scales.'
plot_linechart <- function (y_start = 0, y_end = 100, thumbnail = 0) {
  rolling_period = 5
  data <- as_tibble(raw_data %>%
    filter(!is.na(bet)) %>%
    group_by(jahr) %>%
    ...
    ungroup() %>%
    rollapply(rolling_period, mean, align = 'right'))
  plot <- ggplot(data, mapping = aes(x = jahr, y = avg_bet)) +
    geom_line() +
    xlim(1879, 2020) +
    ylim(y_start, y_end) +
    labs(
...
plot_linechart(35, 75, thumbnail = 1)
plot_linechart(thumbnail = 1)
```
